...
func AKSLocalDNSGenerateCoreFile(
	...
	variables := getCustomDataVariables(config)
	bakerFuncMap := getBakerFuncMap(config, parameters, variables)
	akslocalDNSCorefileTemplate := template.Must(template.New("akslocaldnscorefile").Funcs(bakerFuncMap).Parse(tmpl))

	var b bytes.Buffer
	if err := akslocalDNSCorefileTemplate.Execute(&b, profile.AKSLocalDNSProfileWithSortedDomains); err != nil {
		return "", fmt.Errorf("failed to execute aks-local-dns corefile template: %w", err)
	}

	return base64.StdEncoding.EncodeToString(b.Bytes()), nil
}

// Template to create corefile that will be used by aks-local-dns systemd service.
const akslocalDNSCoreFileTemplateString = `
# whoami (used for health check of DNS)
health-check.aks-local-dns.local:53 {
    bind {{$.AKSLocalDNSProfile.NodeListenerIP}} {{$.AKSLocalDNSProfile.ClusterListenerIP}}
    whoami
}
# VNET DNS traffic (Traffic from pods with dnsPolicy:default or kubelet)
{{- range $index, $domain := .SortedVnetDNSOverrideDomains}}
{{- $override := index $.AKSLocalDNSProfile.VnetDNSOverrides $domain}}
{{$domain}}:53 {
    {{$override.LogLevel}}
    bind {{$.AKSLocalDNSProfile.NodeListenerIP}}
    ...
    forward . {{$.DefaultUpstreamDNSServerIP}} {
        {{- if $override.ForceTCP}}
        force_tcp
        {{- end}}
        policy {{$override.ForwardPolicy}}
    ...
    ready {{$.AKSLocalDNSProfile.NodeListenerIP}}:8181
    ...
    loop
    nsid aks-local-dns
    prometheus {{$.AKSLocalDNSProfile.NodeListenerIP}}:9253
...
{{- $override := index $.AKSLocalDNSProfile.KubeDNSOverrides $domain}}
{{$domain}}:53 {
    {{$override.LogLevel}}
    bind {{$.AKSLocalDNSProfile.ClusterListenerIP}}
